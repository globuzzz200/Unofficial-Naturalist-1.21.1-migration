...
public class Lizard extends TamableAnimal implements NaturalistGeoEntity {
    ...
    private static Ingredient getTemptIngredient() {
        if (temptIngredient == null) {
            temptIngredient = Ingredient.of(
                net.minecraft.world.item.Items.SPIDER_EYE
            );
        }
        return temptIngredient;
    }
    
    public static void refreshIngredients() {
        temptIngredient = null;
    }
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(3, new LizardTemptGoal(this, 0.6, getTemptIngredient(), true));
    ...
    public InteractionResult mobInteract(@NotNull Player player, InteractionHand hand) {
        ...
        if (this.level().isClientSide) {
            boolean bl = this.isOwnedBy(player) || this.isTame() || getTemptIngredient().test(stack) && !this.isTame();
            return bl ? InteractionResult.CONSUME : InteractionResult.PASS;
        }
        if (this.isTame()) {
            if (getTemptIngredient().test(stack) && this.getHealth() < this.getMaxHealth()) {
        ...
        if (!getTemptIngredient().test(stack)) return super.mobInteract(player, hand);
        if (!player.getAbilities().instabuild) {
            stack.shrink(1);
        }
    ...
    private <E extends Lizard> PlayState predicate(final AnimationState event) {
